  ...
  "onSuccess": {
    ...
    callback: async (response, data) => {
      ...
      if (data.length > 0) {
        ...
        let result_searchterm = document.getElementById('search').value.toLowerCase() == ''? '':' for <b>"' + document.getElementById('search').value.toLowerCase() + '"</b>';
...
// Maptiler
 ...
 maptilersdk.config.apiKey = 'fsCLuIQWGPlRskWhImQz';
    ...
    console.log("data " + data);
            ...
            map.loadImage(
                googleBucketUrl + '/map/restaurant+walk.png',
                (error, image) => {
                    if (error) throw error;
                    ...
                    map.loadImage(
                        googleBucketUrl + '/map/r-cluster.png',
                        (error, image) => {
                            if (error) throw error;
